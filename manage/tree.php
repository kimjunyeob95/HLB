...
$query = "select * from tbl_relation_group trg  
            ...
            where emc.mc_position in(select tp_seq from tbl_position where tp_title='대표이사' and tp_coseq={$mc_coseq}) and tg_coseq = {$mc_coseq} and emb.mm_status='Y' and tpt.tp_title='대표이사' and emb.mm_super_admin='F' and emb.mm_is_del='FALSE' and trg.trg_coseq = {$mc_coseq} and emc.mc_coseq =  {$mc_coseq} and tg_parent_seq <> 0 group by mmseq order by tp_regdate desc";
$ps = pdo_query($db,$query,array());
$list_CEO = array();
while($data = $ps ->fetch(PDO::FETCH_ASSOC)){
    array_push($list_CEO,$data);
}
// echo('<pre>');print_r($list_CEO);echo('</pre>');exit;
$query = "select *,CASE WHEN teg.tg_mms_mmseq = emb.mmseq
        ...
        left join ess_member_base emb on trg_mmseq = emb.mmseq
        left join tbl_ess_group teg on trg_group = teg.tg_seq
        where tg_coseq = {$mc_coseq} and emb.mm_super_admin='F' and emb.mm_is_del='FALSE' and emb.mm_status='Y' and trg.trg_coseq = {$mc_coseq} and emc.mc_coseq =  {$mc_coseq} and tg_parent_seq <> 0 order by tg_seq asc";           
$ps = pdo_query($db,$query,array());
$list = array();
while($data = $ps ->fetch(PDO::FETCH_ASSOC)){
...
if(empty($list) || empty($tg_seq) ){
    // page_move('./tree.php','조직이 비어있습니다.');
    echo('<script>');print_r('alert("조직이 비어있습니다.")');echo('</script>');  
}else{
    $query = "select * from tbl_ess_group where tg_seq in 
                ...
                WHERE checkId IS NOT NULL and tg_coseq = {$mc_coseq} and tg_parent_seq <> 0) group by tg_seq;";
    $list_group = array();
    $ps = pdo_query($db,$query,array());
    while($data = $ps ->fetch(PDO::FETCH_ASSOC)){
		...
		<section class="panel panel-featured col-md-12 " style="border:none;">
		    <section role="main" class="content-body">
                ...
                <div class="row">
                    ...
                    <h2 class="content-title" style="height: 68px;"><?=$data_parent['co_name']?> <?if(!empty($data_parent['co_subname'])){ echo '('.$data_parent['co_subname'].')&nbsp;'; }?>조직도</h2>
                        <div class="tab-wrap">
                            <span class="aside-text"><?=$data_parent['co_name']?> <?if(!empty($data_parent['co_subname'])){ echo '('.$data_parent['co_subname'].')&nbsp;'; }?>조직도입니다. </span>
                            <ul class="tab">
...
<?php include $_SERVER['DOCUMENT_ROOT'] . '/manage/include/footer.php'; ?>
 <script>
    $('#form_select').change(function(){
        location.href="./tree.php?mc_coseq="+$(this).val();
    });
	$(document).ready(function(){
        ...
        <?if(!empty($list_CEO)){?>
            ...
            <?for($i=0;$i<sizeof($list_CEO);$i++){
                    if($i==0){
            ?>
                { id: "<?=$data_parent['tg_seq']+$count?>", pid: "CEO", tags: ["대표이사"], 성명: "<?=$enc->decrypt($list_CEO[$i]['mm_name'])?>", 직급 : "<?=get_position_title_type_admin($db,$list_CEO[$i]['mc_position2'],2,$mc_coseq)?>(대표이사)", 직군 : "<?=get_position_title_type_admin($db,$list_CEO[$i]['mc_position3'],3,$mc_coseq)?>" , img: "<?=$list_CEO[$i]['mm_profile']?>",입사일 : '<?=substr($list_CEO[$i]['mc_regdate'],0,10)?>' },
            <?}else{?>
                { id: 1+<?=$i?>, pid: "CEO", tags: ["대표이사"], 성명: "<?=$enc->decrypt($list_CEO[$i]['mm_name'])?>", 직급 : "<?=get_position_title_type_admin($db,$list_CEO[$i]['mc_position2'],2,$mc_coseq)?>(대표이사)", 직군 : "<?=get_position_title_type_admin($db,$list_CEO[$i]['mc_position3'],3,$mc_coseq)?>" , img: "<?=$list_CEO[$i]['mm_profile']?>",입사일 : '<?=substr($list_CEO[$i]['mc_regdate'],0,10)?>' },
            <?}}?>
            );
        <?}else{?>
            child_list.push({ id: "<?=$data_parent['tg_seq']?>", tags: ["<?=$data_parent['tg_seq']?>"], 성명: "<?=$data_parent['co_name']?>"});
        <?}?>
    ...
    child_list.push({ id: "<?="i".$i?>", stpid: "<?=$list[$i]['tg_seq']?>", 성명: `
    <? if($list[$i]['teammanage'] == 1){
        echo $enc->decrypt($list[$i]['mm_name'])." (팀장)";
    }else{
        echo $enc->decrypt($list[$i]['mm_name']);
    }?>`, 직급 : "<?=get_position_title_type_admin($db,$list[$i]['mc_position2'],2,$mc_coseq)?>", 직군 : "<?=get_position_title_type_admin($db,$list[$i]['mc_position3'],3,$mc_coseq)?>" , 직무 : "<?=$list[$i]['mc_job']?>" , img: "<?=$list[$i]['mm_profile']?>",입사일 : '<?=substr($list[$i]['mc_regdate'],0,10)?>' });
    <?}?>
