...
$where .=" and emb.mm_is_del='FALSE' and emb.mm_super_admin='F' and emb.mm_status='Y' and emc.mc_hass = 'F' ";
$query="SELECT COUNT(mmseq) as cnt FROM ess_member_base emb join ess_member_code emc on emb.mmseq = emc.mc_mmseq WHERE 
mmseq not in(select distinct tg_mms_mmseq from tbl_ess_group where tg_mms_mmseq  <> 0) ".$where;
...
//echo('<pre>');print_r($member_list_all);echo('</pre>');
//Param set
	...
	<div class="inner-wrapper">
		...
		<section role="main" class="content-body">
					<header class="page-header">
						<h2> <i class="fas fa fa-user-circle"></i> ESS 사원 </h2>
...
function detail_page(mc_coseq,mseq){
    location.href="/manage/memberDetail.php?mccoseq="+mc_coseq+"&mmseq="+mseq+"&page="+page;
}
...
$('#btn-re-research').click(function(e){
	e.preventDefault();
	location.href="/manage/member.php";
});
