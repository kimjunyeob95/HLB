...
$query="SELECT COUNT(nseq ) as cnt FROM tbl_template_page 
				 ".$where;
			...
			<div class="row">
				<section class="panel panel-featured col-md-12 " style="border:none;">
					...
					<div class="panel-body  col-md-12 " style="display: block;">
						<form class="form-horizontal" id="form_proccess" action="/manage/template.php" method="get">
					...
					<div class="panel-body">
						<table class="table table-bordered table-striped  table-hover mb-none" style="table-layout:fixed ">
								...
								<tr>
									...
									<th class="col-sm-3" style="text-align: center !important;">Type</th>
									<th class="col-sm-6" style="text-align: center !important;">제목</th>
							...
							<? if(isset($list) && count($list) > 0) { 
								...
								<tr>
									...
									<td class="center" ><?=$list[$i]['nType']?></td>
									<td class="center" ><?=$list[$i]['nTitle']?></td>
									<td class="center">
										<?if($list[$i]['modiEnable']=='TRUE'){?>
										<button class="btn btn-xs  btn-detail" seq="<?=$list[$i]['nseq']?>"><i class="fa fa-edit"></i> 수정하기</button>
										<?}else{?>
										<button class="btn btn-xs  btn-primary btn-detail-view" seq="<?=$list[$i]['nseq']?>">상세 보기</button>
										<?}?>
									</td>
								</tr>
							<?
						...
						<div class="col-md-12" style="text-align:right; margin-top: 1vw;">
							<button tabindex="-1" class="btn btn-primary" type="button" onclick="location.href='/manage/template_detail.php'" ><i class="fa fa-edit"></i>등록</button>
						</div>
...
<script type="text/javascript">
	$('.btn-detail').click(function(e){
		...
		location.href="/manage/template_detail.php?nseq="+seq;
	});

	$('.btn-detail-view').click(function(e){
		e.preventDefault();
		var seq = $(this).attr('seq');
		location.href="/manage/template_view.php?nseq="+seq;
	});
</script>