...
$appointment_list = get_appointment_log_list($db,$mmseq,$division); // 사외경력
//echo $enc->encrypt("서울시 강남구 역삼동 721-11");
...
<div id="container" class="ehr-main"> <!-- 180124 클래스 추가(닫기버튼 눌렀을 경우 aside-open 추가) -->
	...
	<div id="content" class="content-primary">
        <?if($appointment_list[0]['ea_state']=='A'){?>
        ...
        <?}else{?>
            <h2 class="content-title" style="margin-top:50px;">정보 변경 상세(발령)</h2>
        <?}?>
		<div class="section-wrap">
            <!-- 기본 사항 -->
...
<script>
    ...
    $('[name="bnt_update"]').click(function(e){
        ...
        if(confirm("정보 변경을 "+title+" 하시겠습니까?")){
            hlb_fn_ajaxTransmit("/@proc/hass/member_appointment_update_proc.php", data);
        }
    })

    function fn_callBack(calback_id, result, textStatus){
        if(calback_id=='member_appointment_update_proc'){
