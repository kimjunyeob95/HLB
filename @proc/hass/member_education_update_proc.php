...
$mail_type='';
if($type=='Y') {
    delete_education_data($db, $mmseq, $division);
    insert_education_data($db, $mmseq, $division);
    $mail_type='승인';
}else{
    $mail_type='반려';
}
update_status_education_log($db,$mmseq,$division,$type); // 사외경력

//사유
if(!empty($cause_return)) {
    $query = "insert tbl_cause_of_return set tco_division = {$division}, tco_type='education' , tco_regdate = now() , tco_text = '{$cause_return}' , tco_mmseq = {$mmseq}";
...
sendmail_memberInfo($data_coperation['co_name'],$mm_name,$to_email,'학력사항',$mail_type);
