...
//암호화 대상 필드
$enctArr = array('ep_name','ep_institution');
...
$query = "update ess_paper_log set ep_state = 'C' where ep_mmseq=".$mmseq;
$ps = pdo_query($db,$query,array());
//여러개 신청이 있을경우 대비 (구분)
$query="SELECT ep_division+1 as cnt FROM ess_paper_log WHERE ep_mmseq=".$mmseq." order by ep_division desc limit 1";
$ps = pdo_query($db,$query,array());
$data = $ps->fetch(PDO::FETCH_ASSOC);
if(empty($data)){
    $data['cnt'] = 1;
}
for($i=0;$i<sizeof($_REQUEST['ep_name']);$i++){
    $insertQuery="INSERT INTO  ess_paper_log  SET
					ep_mmseq='".$mmseq."', 
					ep_applydate = now()  ";
    $insertQuery .=  "
        , ep_date = '{$_REQUEST['ep_date'][$i]}'
        , ep_name = '{$_REQUEST['ep_name'][$i]}'
        , ep_institution = '{$_REQUEST['ep_institution'][$i]}'
...
foreach($list_hass as $index => $val){
    ...
    sendmail_essInfo($data_coperation['co_name'],$mm_name,$to_email,'논문 / 저서');
}

    $result['code']='TRUE';
    $result['msg']='논문 / 저서 정보가 수정 요청 되었습니다.';
    echo json_encode($result);
