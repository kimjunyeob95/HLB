$(document).ready(function () {
    ...
    $('.tab li a').each(function () {
        var params = getParameterByName('active');
        var this_href = $(this).attr('href');
        if (params == '') {
            $(this_href).css('display', 'none').siblings('.tab-cont').first().css('display', 'block');
        } else if (params == 'family') {
            $(this_href).css('display', 'none').siblings('.tab-cont').eq(1).css('display', 'block');
        } else if (params == 'project') {
            $(this_href).css('display', 'none').siblings('.tab-cont').eq(8).css('display', 'block');
        }
    ...
    $('.btn-top').click(function (e) {
        ...
        if ($(this).hasClass('btn-scrolldown')) {
            ...
            $('html, body')
                .stop()
                .animate({ scrollTop: offset }, 400, function () {
                    //$('.btn-top').removeClass('btn-scrolldown').700('scroll');
                });
        } else {
            $('html, body')
                .stop()
                .animate({ scrollTop: 0 }, 400, function () {
                    //$('.btn-top').addClass('btn-scrolldown').addClass('scroll');
                });
        }
    });
    ...
    if (navigator.userAgent.indexOf('MSIE') >= 0 || navigator.userAgent.indexOf('Trident') >= 0) {
        // IE가 문제이므로 IE만 동작.
        if ($('body').find('div').hasClass('window-popup')) {
            // IE면서 팝업화면일 경우 가로스크롤 생기는 문제
            //$.logger("팝업 맞다");
