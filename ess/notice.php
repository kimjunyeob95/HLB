...
$addquery='';
if(!empty($search)){
    $addquery .= " and (tn_title like '%{$search}%' or tn_content like '%{$search}%') ";
}
$query = "select count(*) as cnt from tbl_noti_page where tn_is_del = 'F' and tn_status = 'T' and tn_topshow = 'F' {$addquery} and tn_coseq = {$coseq} order by tn_regdate desc";
...
$query = "select * from tbl_noti_page where tn_is_del = 'F' and tn_status = 'T' and tn_topshow = 'F' {$addquery} and tn_coseq = {$coseq} order by tn_regdate desc limit {$from} , {$rows} ";
            ...
            <div class="table-wrap">
                <table class="data-table" id="tb_insaHeader">
                    ...
                    <?for($i=0;$i<sizeof($list_toplist);$i++){?>
                        <tr onclick="move_detail_ajax(<?=$list_toplist[$i]['tn_seq']?>,'<?=$subquery?>')" style="cursor: pointer">
                            <td>공지</td>
                            <td class="center"><?=$list_toplist[$i]['tn_title']?></td>
                            <td class="center"><?=$enc->decrypt(get_member_info($db,$list_toplist[$i]['tn_last_admin'])['mm_name'])?></td>
                            <td><?=substr($list_toplist[$i]['tn_regdate'],0,10)?></td>
                            <td><?=$list_toplist[$i]['tn_views']?></td>
                        </tr>
                    <?}?>
                    ...
                    <?}else{?>
                        <?for($i=0;$i<sizeof($list);$i++){?>
                            <tr onclick="move_detail_ajax(<?=$list[$i]['tn_seq']?>,'<?=$subquery?>')" style="cursor: pointer">
                                <td><?=$numbering?></td>
                                <td class="center"><?=$list[$i]['tn_title']?></td>
                                <td class="center"><?=$enc->decrypt(get_member_info($db,$list[$i]['tn_last_admin'])['mm_name'])?></td>
                                <td><?=substr($list[$i]['tn_regdate'],0,10)?></td>
                                <td><?=$list[$i]['tn_views']?></td>
                            </tr>
                        <?$numbering--;}?>
                    <?}?>
                    </tbody>
...
function move_detail_ajax(seq,subquery){
    var data = { 'tn_seq' : seq ,'subquery' : subquery , 'type': 'notice'};
...
function fn_callBack(calback_id, result, textStatus){
    if(calback_id=='noticeViewProc'){
        if(result.code=='FALSE'){
            ...
            location.href="/ess/noticeDetail?seq="+result.seq+result.subquery;
        }
    }
}
</script>
