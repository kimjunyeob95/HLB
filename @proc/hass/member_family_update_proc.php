...
$mail_type='';
if($type=='Y') {
    delete_family_data($db, $mmseq, $division);
    insert_family_data($db, $mmseq, $division);
    $mail_type='승인';
}else{
    $mail_type='반려';
}
update_status_family_log($db,$mmseq,$division,$type); // 사외경력

if(!empty($cause_return)) {
    $query = "insert tbl_cause_of_return set tco_division = {$division}, tco_type='family' , tco_regdate = now() , tco_text = '{$cause_return}' , tco_mmseq = {$mmseq}";
...
sendmail_memberInfo($data_coperation['co_name'],$mm_name,$to_email,'가족사항',$mail_type);
