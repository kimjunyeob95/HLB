...
$query = " select * from tbl_appointment ta  where ta_seq = {$ta_seq} ";
...
<div id="container" class="ehr-main"> <!-- 180124 클래스 추가(닫기버튼 눌렀을 경우 aside-open 추가) -->
	...
	<div id="content" class="content-primary">
        <h2 class="content-title">인사발령(<?=$title?>) 받은내역 상세관리</h2>
        <form id="info_insert_form">
            <input type="hidden" name ="ta_prev_coseq" value="<?=$data['ta_prev_coseq']?>">
                    ...
                    <div class="table-wrap" style="margin-top:20px;border-top:1px solid #d1d1d1;">
                        <table class="data-table left">
                            ...
                            <tr>
                                ...
                                <td><input type="text" class="input-text input-datepicker"  title="그룹 입사일" name="mc_affiliate_date" style="max-width:50%;" value="<?=substr($member_info['mc_affiliate_date'],0,10)?>"></td>
                                <th scope="col">자사 입사일</th>
                                <td><input type="text" class="input-text input-datepicker"  title="자사 입사일" name="mc_regdate" style="max-width:50%;" value="<?=substr($member_info['mc_regdate'],0,10)?>"></td>
                                <th scope="col">최종 승진일</th>
                                <td><input type="text" class="input-text input-datepicker" title="최종 승진일" name="mc_bepromoted_date" style="max-width:50%;" value="<?=substr($member_info['mc_bepromoted_date'],0,10)?>"></td>
                            </tr>
                            <tr>
                        ...
                        <table class="data-table table-family-list left">
                            ...
                            <?foreach ($family_list as $val){?>
                                <tr>
                                    ...
                                    <td>
                                        <?=$family_type_array[$val['mf_relationship']]?>
                                    </td>
...
<script>
    ...
    //입력
    $('[name="btn_save"]').click(function(e){
        e.preventDefault();
        var validate = true;
        confirm_type = $(this).data('type');

        group = $('input[name="mm_group[]"]').length;
        if(group=='' && confirm_type =='Y'){
            alert('소속을 선택해주세요.');
            return false;
        }
        ...
        type = '<?=$type?>';
        seq = '<?=$seq?>';
        ta_prev_coseq = $('[name="ta_prev_coseq"]').val();
        position = $('[name="mc_position"] option:selected').val();
        position2 = $('[name="mc_position2"] option:selected').val();

        data = $('#info_insert_form').serialize()+ "&mmseq="+seq + "&type="+type+ "&confirm="+confirm_type+"&ta_prev_coseq="+ta_prev_coseq+"&ta_seq="+<?=$ta_seq?>;
        //data = {'mmseq':seq ,'type':type,'confirm':confirm_type,'ta_prev_coseq':ta_prev_coseq,'group':group,'position':position,'position2':position2,'ta_seq':<?=$ta_seq?>};
        title = $(this).data('btn');
        if(validate) {
            if (confirm("정말로 " + title + " 하시겠습니까?")) {
                hlb_fn_ajaxTransmit("/@proc/hass/appointment_update_proc.php", data);
            }
        }
    })
