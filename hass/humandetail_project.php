...
$project_list = get_project_log_list($db,$mmseq,$division); // 사외경력
//echo $enc->encrypt("서울시 강남구 역삼동 721-11");
...
<div id="container" class="ehr-main"> <!-- 180124 클래스 추가(닫기버튼 눌렀을 경우 aside-open 추가) -->
    ...
    <h2 class="content-title">정보 변경 상세(프로젝트)</h2>
                ...
                <div class="table-wrap" style="margin-top:20px;border-top:1px solid #d1d1d1;">
                    <table class="data-table left">
                        ...
                        <?foreach ($project_list as $key =>$val){?>
                            <tr>
                                <td class="insert"><?=substr($val['epj_sdate'],0,10)?></td>
                                <td class="insert"><?=substr($val['epj_edate'],0,10)?></td>
                                <td class="insert"><?=$enc->decrypt($val['epj_name'])?></td>
                                <td scope="insert"><?=$val['epj_result']?></td>
                                <td scope="insert"><?=$val['epj_position']?></td>
                                <td scope="insert"><?=$val['epj_result']?></td>
                                <td scope="insert"><?=$val['epj_content']?></td>
                                <td scope="insert"><?=$val['epj_keyword']?></td>
                            </tr>
                        <?}?>
                        </tbody>
                    </table>
                        ...
                        <colgroup>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td><input class="input-text" type="text" name=""></td>
                                </tr>
                            </tbody>
                    </table>
                </div>
...
<script>
    ...
    $('[name="bnt_update"]').click(function(e){
        ...
        title = $(this).data('btn');
        e.preventDefault();
        var data = {'type':type,'mmseq':<?=$mmseq?>,'division':<?=$division?>};
        if(confirm("정보 변경을 "+title+" 하시겠습니까?")){
            hlb_fn_ajaxTransmit("/@proc/hass/member_project_update_proc.php", data);
        }
    })

    function fn_callBack(calback_id, result, textStatus){
        if(calback_id=='member_project_update_proc'){
